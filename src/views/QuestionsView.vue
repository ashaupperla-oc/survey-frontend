    ...
    <div v-for="(question, index) in questions" :key="index"   >
      ...
      <div class="card read-only-question cursor_grab" v-if="selectedQuestion.id !== question.id">
        <div class="row container-fluid" style="display:flex;">
            ...
            <div class="answer-section">
              <div class="option-section" v-if="question.type === 'BOOLEAN'">
                <div class="" v-for='(option, optionIndex) in question.options' :key="optionIndex">
                  <p class="radio-option">
                    <input type="radio" :name="'boolean_type'+index" :disabled="readOnly" :value="option.body">
                    <label>{{option.body}}</label>
                  </p>
                </div>
              </div>
              <div class="option-section pad-top20" v-if="question.type === 'SCALE'">
                <StarRating :max-rating="question.intervals" />
              </div>
              <div class="option-section" v-if="question.type === 'TEXT'">
                <input type="text" :name="textinputname" class="input-text readonly" placeholder=""  :readonly="readOnly" />
              </div>
              ...
              <div class="option-section" v-if="question.type === 'MULTI_CHOICE'">
                <div v-for='(option, optionIndex) in question.options' :key="optionIndex">
                  ...
                  <label>
                    <input type="checkbox" :name="'multi'+ index+' '+optionIndex" :disabled="readOnly" :value="option.body">&nbsp;{{option.body}}
                  </label>
                </div>
              </div>
            </div>
          ...
          <div v-if="!enduser" class="p-0 text-right" style="display:flex;">
            <button type="button" class=" btn btn-sm btn-danger" v-on:click="editQuestion(question, index)">Edit</button>
            <div style="width:1%"/>
            <button type="button" class="btn btn-sm btn-danger" v-on:click="deleteQuestion(question, index)">Delete</button>
          </div>
      ...
      <div v-if="enduser" class="p-0 text-right">
        <button type="button" class="sb-btn-link mr-10 color-orange" v-on:click="submitanswer(questions)">Submit</button>
      </div>
  </div>
</template>
...
export default {
  ...
  components: { SurveyBuilder, StarRating },
  mounted() {
